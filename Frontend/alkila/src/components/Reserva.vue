<template>
    <div id="Historial">
        ...
        <div class="container">
            ...
            <h2>
                Saldo:
                <span>${{ reservaByUsername.balance }} COP</span>
            </h2>
            <h2>
                Último Movimiento:
                <span>
                    {{ reservaByUsername.lastChange.substring(0, 10) }}
                    {{ reservaByUsername.lastChange.substring(11, 19) }}
                </span>
            </h2>
        </div>
...
export default {
    ...
    apollo: {
        ...
        reservaByUsername: {
            query: gql`
                query ($username: String!) {
                    reservaByUsername(username: $username) {
                        balance
                        codigocasa
                        tiempoestadia
                        lastChange
    ...
    created: function () {
        ...
        this.$apollo.queries.reservaByUsername.refetch();
    }
};
</script>
